...
Trang thai production da chot xong ben Web tong:
1. Web tong da dat AI_APP_SHARED_KEY that tren production
2. Endpoint production /api/ai-app/* da tra dung contract verify:
   - features[]
   - grace { allowed, graceDays, offlineUntil }
3. AI-app chi con viec nhan key qua kenh secure, set WEB_TOTAL_AI_APP_KEY ben minh va goi that vao production
...
Noi ngan gon: phia Web tong da xong production that. Phia AI-app khong con blocker ben Web tong; viec con lai la set env WEB_TOTAL_AI_APP_KEY va goi that vao production.
```